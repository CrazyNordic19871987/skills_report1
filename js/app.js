// =============================================
//  ONE! Summer Game 2026 — Лагерь будущего 2026
// =============================================
...
function renderRadarChart(scores) {
  ...
  ctx.fillStyle = grad;
  ctx.fill();
  ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--orange').trim() || '#ed7615';
  ctx.lineWidth = 2;
  ctx.stroke();
  ...
  COMPETENCIES.forEach((c, i) => {
    ...
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--orange').trim() || '#ed7615';
    ctx.fill();
  });
}
...
function renderDISC(obs) {
  ...
  const colors = DISC_COLORS;

  const labels = {
    D:{label:'Доминирование', color:colors.D, desc:'Активный, решительный'},
    I:{label:'Влияние',       color:colors.I, desc:'Общительный, креативный'},
    S:{label:'Стабильность',  color:colors.S, desc:'Уравновешенный, надёжный'},
    C:{label:'Постоянство',  color:colors.C, desc:'Аналитик, точный'}
  };
  ...
  const comboHtml = '<div class="disc-combo-section"><h3 style="font-size:0.8rem;color:var(--muted);margin:12px 0 8px;font-weight:600;">Комбо-типы DISC</h3>' +
...
function renderCampPage() {
  ...
  if (tracksContainer) {
    tracksContainer.innerHTML = `
      <h3 style="font-size:0.85rem;color:var(--orange);margin-bottom:12px">🎯 Треки профессиональной ориентации</h3>
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px">
        ${CAMP_CLUBS.map(club => `
          <div class="gc">
            ...
            ${!club.active ? '<div style="font-size:0.65rem;color:var(--orange);background:var(--orange-dim);padding:2px 8px;border-radius:10px;display:inline-block;margin-top:6px">🔒 Откроется в следующей смене</div>' : ''}
          </div>
        `).join('')}
      </div>
      <table style="width:100%;margin-top:12px;font-size:0.75rem;color:var(--muted)">
        <tr><th style="text-align:left;color:var(--white)">Клуб</th><th style="text-align:left;color:var(--white)">Описание</th><th style="text-align:left;color:var(--white)">Доступность</th></tr>
        <tr><td>Спорт</td><td>Бадминтон, теннис, плавание и фитнес</td><td>✓</td></tr>
        <tr><td>Творчество</td><td>Рисование, лепка, музыка</td><td>✓</td></tr>
        <tr><td>Настольные игры</td><td>Монополия, UNO, настолки и др.</td><td>Доступен после лагеря</td></tr>
        <tr><td>Киберспорт</td><td>FIFA, Gran Turismo, Nintendo Wii, Roblox, Minecraft, Just Dance, PlayStation</td><td>Доступен после лагеря</td></tr>
      </table>
    `;
  }
